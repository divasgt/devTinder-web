...
function UserProfileSidebar({ user, connectionData = null, currentUserId = null, isSelf = false }) {
  ...
  const handleRemoveConnection = async () => {
    try {
      await axios.post(`${BASE_URL}/request/remove/user/${_id}`, {}, { withCredentials: true });
  ...
  if (isSelf) {
    ...
    actionUI = (
      <div className="flex flex-col gap-2">
        <p className="text-sm font-medium text-fg-muted -mt-2">Sent you a request</p>
        <div className="flex gap-2">
          <button onClick={() => handleReview("rejected", reqId)} className="btn-ghost flex-1">
            Reject
          </button>
          <button onClick={() => handleReview("accepted", reqId)} className="btn-primary flex-1">
            Accept
          </button>
        </div>
      </div>
    ...
    actionUI = (
      <div className="flex flex-col gap-2">
        <p className="text-sm font-medium text-fg-muted -mt-2">You rejected this user</p>
        <div className="flex gap-2">
          <button onClick={() => handleSendRequest("ignored")} className="btn-ghost flex-1">
            Ignore
          </button>
          <button onClick={() => handleSendRequest("interested")} className="btn-primary flex-1">
            Connect
          </button>
        </div>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        ...
        <div className="relative size-80 w-full overflow-hidden bg-gradient-to-br from-indigo-500/30 via-purple-500/30 to-rose-500/30">
          {photoUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center">
              <span className="font-bold text-8xl text-white/80 select-none">{initial}</span>
            </div>
          )}
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          {socialLinks.length > 0 && (
            <div className={contactEmail ? "" : "pt-5 border-t border-surface-2"}>
                  ...
                  return (
                    <li key={idx}>
                      ...
                      >
                        <span className="text-fg-muted group-hover:text-primary transition-colors">
                          <Icon
                            name={(link.title || "").toLowerCase().trim()}
                            className="w-4 h-4"
                          />
                        </span>
